...
async def test_viral_pipeline():
    ...
    try:
        ...
        for i, clip in enumerate(results[:2]):
            logger.info(
                f"Clip {i+1}: {clip.reason} | Viral Score: {clip.viralAnalysis.score}"
            )
            logger.info(
                f"Visual Energy: {clip.viralAnalysis.visualEnergy} (Vision-grounded)"
            )
        return True
...
async def test_director_pipeline():
    ...
    try:
        ...
        logger.info(
            f"Director Success: Storyboard generated with {len(result.get('storyboard', []))} scenes"
        )
...
async def test_preflight_pipeline():
    ...
    try:
        ...
        result = await run_preflight_pipeline(
            source_url, test_clips, False, "test_user_123"
        )
        ...
        logger.info(
            f"Pre-Flight Success: Weighted Consensus Score: {res_dict.get('weighted_consensus_score')}"
        )
