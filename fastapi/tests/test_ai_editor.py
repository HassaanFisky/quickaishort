...
def test_all_23_action_types_parse():
    ...
    samples: list[dict[str, Any]] = [
        ...
        {"type": "PAUSE"},
        {"type": "EXPORT_CLIP"},
        {"type": "ADD_ELEMENT", "element": {"type": "TEXT", "text": "hi", "x": 540, "y": 960, "scale": 1, "rotation": 0, "color": "#fff"}},
...
def test_sanitise_update_element_clamps_position():
    state = make_state(videoDuration=30.0)
    actions: list[AiEditorAction] = [
        UpdateElementAction(type="UPDATE_ELEMENT", id="el1", patch={"x": 9999.0, "color": "#fff"}),
    ]
    safe, clamped, dropped = sanitise(actions, state)
    assert len(safe) == 1
...
async def test_engine_returns_noop_on_parse_failure():
    state = make_state(videoDuration=60.0)
    with patch("services.ai_editor_engine.call_gemini_text", new=AsyncMock(return_value="not json!!!")):
