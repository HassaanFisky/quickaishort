...
class ScriptAgent:
    ...
    async def enhance_script(self, topic: str) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                client.models.generate_content, model=DEFAULT_MODEL, contents=prompt
            )
    ...
    def find_stock_video(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            if "videos" in data and len(data["videos"]) > 0:
                ...
                best_file = next(
                    (f for f in video_files if f.get("width") == 1080), video_files[0]
                )
    ...
    async def run(self, user_script: str, clip_paths: List[str]) -> Dict[str, Any]:
        ...
        for seg in segments:
            ...
            final_segments.append({**seg, "clip_path": clip_path})
