...
The `VideoCanvas` component wires a **Web Audio API chain** — `MediaElementSource → BiquadFilter (highpass 80 Hz) → GainNode → destination` — giving live noise suppression and audio boost directly in the preview. A Gemini-backed AI panel accepts natural language instructions ("trim the first 4 seconds", "add a text overlay at 0:12") and dispatches them as typed editor actions.

The final export path skips all of this and runs server-side: `POST /api/process-video` → RQ render worker → FFmpeg → GCS.
...
| **Database** | MongoDB Atlas · GridFS | Credits, job history, user stats; legacy `/api/v1/video` path |
...
| **Deploy** | Vercel (frontend) · Cloud Run (API + Worker) | `quickai-api` + `quickai-worker` services; startup probe tuned for 13 s ADK import |
...
│   ├── src/components/       # Editor panels, VideoCanvas, AIPanel
