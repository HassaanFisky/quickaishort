...
# 2. Ensure the durable wake path and least-privilege identities exist.
echo "Configuring Cloud Tasks queue and IAM..."
...
echo "Deploying request-bound renderer..."
gcloud run deploy quickai-worker \
    ...
    --max-instances 3 \
    --cpu-throttling \
    "--liveness-probe=httpGet.path=/health/live,timeoutSeconds=5,failureThreshold=3,periodSeconds=30" \
    "--startup-probe=httpGet.path=/health/live,timeoutSeconds=5,failureThreshold=20,periodSeconds=5" \
    --project ${PROJECT_ID} \
    --update-env-vars \
...
RENDER_URL=$(gcloud run services describe quickai-worker \
...
CLOUD_TASKS_RENDER_URL=${RENDER_URL},\
CLOUD_TASKS_OIDC_AUDIENCE=${RENDER_URL},\
...
echo "Private renderer URL: ${RENDER_URL}"
