  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isActive
          ? "nano-glass border border-border shadow-[0_0_30px_rgba(0,0,0,0.15)]"
          : "opacity-40",
      )}
    >
      {/* Node Visual */}
      ...
      <div className="flex-1 flex flex-col gap-1 min-w-[200px]">
        ...
        <div className="h-8 relative overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.p
              ...
              exit={{ y: -12, opacity: 0 }}
              className="text-[9px] font-medium text-muted-foreground italic flex items-center gap-2"
            >
              {agent.status === "working" && (
        ...
        {agent.reasoningLogs && agent.reasoningLogs.length > 0 && (
          <div className="mt-2 p-2 bg-black/20 rounded-xl max-h-24 overflow-y-auto custom-scrollbar border border-border shadow-inner">
        ...
        {/* Mini Progress Bar */}
        <div className="w-full h-1 bg-foreground/10 rounded-full overflow-hidden mt-1 shadow-inner">
          <motion.div
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-8 gap-8">
      ...
      <div className="relative w-full max-w-2xl flex flex-col gap-4">
        <div className="absolute left-[32px] top-[48px] bottom-[48px] w-px bg-gradient-to-b from-primary/50 via-primary/10 to-transparent" />
      ...
      {/* Footer */}
      <div className="flex items-center gap-4 mt-4 px-6 py-3 nano-glass rounded-2xl border border-border opacity-60 hover:opacity-100 transition-opacity">
