...
export const AI_TOOL_CATALOG: AiTool[] = [
  ...
  {
    id: "set-clip-gain",
    name: "Audio · Clip Gain",
    description: "Adjust the output gain (in dB) for the selected clip on the audio mix bus.",
    category: "Audio",
    iconName: "Volume2",
    keywords: ["gain", "volume", "clip", "db", "audio", "mix", "level"],
    execMode: "gemini",
    isEnabled: (s) => !!s.selectedClipId,
    buildActions: (s) => [{
      type: "SET_CLIP_GAIN",
      payload: { clip_id: s.selectedClipId ?? "", gain_db: 0 },
    }],
  },
  {
    id: "set-master-gain",
    name: "Audio · Master Gain",
    description: "Adjust the master output gain (in dB) for the entire mix.",
    category: "Audio",
    iconName: "SlidersHorizontal",
    keywords: ["master", "gain", "volume", "db", "output", "mix"],
    execMode: "gemini",
    isEnabled: () => true,
    buildActions: () => [{ type: "SET_MASTER_GAIN", payload: { gain_db: 0 } }],
  },
  {
    id: "enable-denoise",
    name: "Audio · RNNoise Denoise",
    description: "Enable or disable RNNoise AI noise suppression on the selected clip.",
    category: "Audio",
    iconName: "Mic",
    keywords: ["denoise", "noise", "rnnoise", "suppress", "clean", "speech"],
    execMode: "direct",
    isEnabled: (s) => !!s.selectedClipId,
    buildActions: (s) => [{
      type: "ENABLE_DENOISE",
      payload: { clip_id: s.selectedClipId ?? "", enabled: true },
    }],
  },
  {
    id: "enable-limiter",
    name: "Audio · Limiter",
    description: "Enable the brick-wall look-ahead limiter on the master bus to prevent clipping.",
    category: "Audio",
    iconName: "ShieldCheck",
    keywords: ["limiter", "brick wall", "clip", "master", "peak", "loudness"],
    execMode: "direct",
    isEnabled: () => true,
    buildActions: () => [{ type: "ENABLE_LIMITER", payload: { enabled: true } }],
  },
  {
    id: "add-fade-in",
    name: "Audio · Fade In",
    description: "Add a linear fade-in envelope to the selected clip.",
    category: "Audio",
    iconName: "TrendingUp",
    keywords: ["fade", "in", "envelope", "ramp", "smooth", "start"],
    execMode: "direct",
    isEnabled: (s) => !!s.selectedClipId,
    buildActions: (s) => [{
      type: "ADD_FADE_IN",
      payload: { clip_id: s.selectedClipId ?? "", duration_ms: 500 },
    }],
  },
  {
    id: "add-fade-out",
    name: "Audio · Fade Out",
    description: "Add a linear fade-out envelope to the selected clip.",
    category: "Audio",
    iconName: "TrendingDown",
    keywords: ["fade", "out", "envelope", "ramp", "smooth", "end"],
    execMode: "direct",
    isEnabled: (s) => !!s.selectedClipId,
    buildActions: (s) => [{
      type: "ADD_FADE_OUT",
