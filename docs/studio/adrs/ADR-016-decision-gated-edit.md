...
5. **Verification owns execution/result integrity.** Completing a plan, HTTP 200, or creating a file is **not** proof the objective succeeded. M0 records a verification plan only; Tier 0 execute wiring is a later task.
...
In-repo contracts + deterministic `resolve_objective` + unit tests.

Out of scope: frontend, HTTP router, Orchestrator `decision_id` wiring, Pre-Flight integration, new agents, semantic pacing, learning/calibration, live Gemini.
